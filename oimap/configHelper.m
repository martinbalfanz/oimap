...
+ (BOOL)doesConfigFileExist {
    return [[NSFileManager defaultManager] fileExistsAtPath:[self getConfigPath] isDirectory:false];
}
...
+ (void)createConfigFile {
    ...
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:defaultData options:NSJSONWritingPrettyPrinted error:&error];
...
+ (NSDictionary *)readConfigFile {
    ...
    NSDictionary *configData = [NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:&error];
...
+ (BOOL)saveConfigFile:(NSDictionary *)config {
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:[self getConfig] options:NSJSONWritingPrettyPrinted error:&error];
